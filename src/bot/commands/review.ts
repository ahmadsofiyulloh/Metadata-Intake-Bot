...
export async function handleReviewCommand(): Promise<OutboundMessage[]> {
  const drafts = await listDraftsByPredicate(
    (draft) =>
      draft.compliance_status !== "SAFE_TO_DRAFT" ||
      draft.data_status === "DATA_SEBAGIAN" ||
      draft.data_status === "DRAFT",
    { limit: 20, includeArchived: false }
  );

  const messages: OutboundMessage[] = [
    {
      text: formatReviewListMessage("PRODUK PERLU REVIEW", drafts),
      parseMode: "HTML",
